 ...
 */
//Find starred restaurant with matching id
router.get('/:id', (req, res) =>{
  const {id} = req.params;
  const starredRestaurant = STARRED_RESTAURANTS.find((restaurant)=> {
    restaurant.id === id;
    //Check if the starred restaurant exists
    if(!restaurant){
...
res.status(200).json({
  ...
  comment: newStarredRestaurant.null,
  name: restaurant.name
});
})
...
router.delete('/:id', (req, res) => {
  const { id } = req.params;
 const newStarredList = STARRED_RESTAURANTS.filter((restaurant) => restaurant.id !== id);
if(STARRED_RESTAURANTS.lenght === newStarredList.length) {
  res.status(404).send('Do Not Unstar');
  return;
}
